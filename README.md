...
You've successfully created a **dairy-company-data-platform** project using Polster CLI! This README will help you get started quickly and build your data pipeline.
...
```bash
cd dairy-company-data-platform
ls -la
```
...
Your Polster pipeline includes comprehensive error handling and logging:

**Error Handling:**
- User-friendly error messages with actionable troubleshooting steps
- Graceful degradation when cloud storage (ADLS) is unavailable
- Input validation for DataFrames and required columns
- Comprehensive exception handling across all pipeline stages

**Logging & Monitoring:**
- Structured logging with operation timing for performance monitoring
- Console output for development feedback
- Detailed error logging for debugging
- Operation summaries with execution times

**Storage Resilience:**
- Dual writing: Full data to cloud, samples locally for development
- Automatic fallback from ADLS to local storage on connection issues
- Configurable sample sizes for development efficiency
...
ADLS_BASE_PATH=data/dairy-company-data-platform
...
- `DUAL_WRITE_SAMPLE_SIZE`: Number of rows to sample locally (default: 50)
- `DISABLE_LOCAL_SAMPLE`: Set to `true` to disable local sample writing
